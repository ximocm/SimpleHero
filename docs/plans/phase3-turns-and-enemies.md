...
- Deterministic turn order for heroes and enemies.
...
- Use a simple deterministic side-based order for MVP:
  1. Hero 1
  2. Hero 2
  3. Hero 3
  4. Enemy 1..N in stable spawn order
- Skip dead units automatically.
- Each hero turn tracks:
  - `movementRemaining`
  - `attackSlotAvailable`
  - `actionPointsRemaining`
  - temporary turn effects
- End turn advances to the next living unit and applies cooldown/effect expiry rules.
...
- When the last enemy dies:
  ...
  - grant total gold reward for the defeated enemies
  - unblock room traversal/exit behavior
- If all heroes die, trigger run loss state instead of normal exploration.
- Treasure reward flow can remain Phase 4 if combat reward accounting is already isolated.
...
2. Combat turn order is stable and advances correctly through all living heroes and enemies.
...
9. Winning combat marks the room cleared and grants the summed enemy gold reward.
...
- New `src/systems/turnSystem.ts`
  - Own active unit, turn advancement, start/end-turn resets, cooldown ticks.
...
- New `src/systems/combatSystem.ts`
  - Attack resolution, damage, defeat handling, reward hooks.
- `src/systems/persistenceSystem.ts`
  - Save/restore for encounter, enemy, turn, and gold state.
...
2. **M3.2 Turn engine**
   - Hero/enemy initiative order, per-turn resources, end-turn progression.
3. **M3.3 Combat resolution**
   - Basic attacks, damage formulas, defeat handling, reward hooks.
